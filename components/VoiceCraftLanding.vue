...
const quickStartPath = computed(() => locale.value === 'ru' ? '/ru/start/quick-start' : '/en/start/quick-start')
    ...
    <div class="vc-hero vc-reveal rounded-2xl p-6 md:p-10">
      <div class="mx-auto max-w-4xl text-center space-y-6">
        ...
        <div class="flex flex-wrap justify-center gap-3">
          <UButton size="xl" color="primary" trailing-icon="i-lucide-arrow-right" class="vc-btn-main comf-500-20"
            :to="quickStartPath">
            {{ t('landing.quickStart') }}
          </UButton>
          <UButton size="xl" color="neutral" variant="outline" class="vc-btn-outline comf-500-20"
            to="https://github.com/AvionBlock/VoiceCraft/releases/latest" target="_blank">
    ...
    <div class="vc-glass vc-tour vc-reveal rounded-2xl p-5 md:p-8 space-y-5">
      <div class="flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          <UButton class="vc-icon-btn" color="neutral" variant="soft" icon="i-lucide-chevron-left" @click="prevSlide" />
          <UButton class="vc-icon-btn" color="neutral" variant="soft" icon="i-lucide-chevron-right"
            @click="nextSlide" />
        </div>
      </div>
      ...
      <div class="grid gap-6 md:grid-cols-[0.9fr_1.1fr] md:items-stretch">
        <div class="space-y-3 md:pr-4 flex flex-col justify-center">
          ...
          <div class="flex gap-2 pt-2">
            <button v-for="(_, index) in slides" :key="index" class="vc-dot"
              :class="{ 'vc-dot-active': currentSlide === index }" @click="currentSlide = index" />
          </div>
        </div>

        <div class="overflow-hidden rounded-xl border border-white/20 bg-black/30 shadow-2xl">
          <div class="aspect-video">
            <img :key="slides[currentSlide]?.image" :src="slides[currentSlide]?.image"
              :alt="slides[currentSlide]?.title" class="h-full w-full object-cover vc-image-reveal">
          </div>
        </div>
      </div>
    </div>
