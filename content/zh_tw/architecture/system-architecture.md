...
VoiceCraft 是距離感語音堆疊，而不是單一 Minecraft 模組。客戶端負責麥克風音訊，伺服器管理語音會話和共享狀態，Minecraft 整合層告訴伺服器玩家在哪裡以及他們的狀態應如何表示。

這種分離是有意為之的：只要連接了正確的面向 Minecraft 的傳輸，同一語音伺服器就可以與 Bedrock 專用伺服器、本地 Bedrock 世界、直接 Paper 伺服器和代理網路一起使用。

## 主要層

| 圖層 | 主要職責 | 典型安裝位置 |
|-------|---------------------|--------------------------|
| `VoiceCraft.Client` | 擷取麥克風輸入、傳送語音資料包、播放附近的聲音、儲存本機音訊首選項。 | 玩家設備 |
| `VoiceCraft.Server` | 接受語音用戶端、儲存實體狀態、應用審核標誌和音訊效果預設值、公開 Minecraft 傳輸。 | VPS、遊戲主機、本機 PC 或外掛程式管理的執行時間 |
| Minecraft 整合 | 將玩家/實體位置和生命週期資料從 Minecraft 發送到 VoiceCraft。 | Bedrock 插件、Paper 插件或代理插件 |
...
- `VoiceCraft.Addon.Core.McHttp` 用於Bedrock 專用伺服器
- `VoiceCraft.Addon.Core.McWss` 用於本地 Bedrock 世界和指令隧道設置
- `VoiceCraft.Java` 適用於 Java、Geyser/Floodgate、Paper、Velocity 和 BungeeCord 拓樸
...
這種分離可以讓您在更改 Minecraft 整合的同時保持語音伺服器的穩定。例如，僅Bedrock部署可以使用 `McHttp`，而 Java/Geyser 網路可以保留相同的核心語音伺服器，但將 Minecraft 端切換到 `McTcp`。
