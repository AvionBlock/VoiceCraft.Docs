...
- 基岩專用伺服器：讀取[McHttp for BDS](/zh_tw/minecraft/mchttp-bds)
- Java + Geyser 伺服器：讀取 [GeyserVoice](/zh_tw/ecosystem/geyservoice)

## 1.下載伺服器

1. 開啟[下載頁面](/zh_tw/download)。
2. 下載適合您平台的伺服器存檔：
...
如果您從原始程式碼構建，請參閱 [VoiceCraft 倉庫和建置](/zh_tw/ecosystem/voicecraft-repository)。
...
有關完整比較，請參閱[傳輸模式](/zh_tw/server/transports)。

## 5.下載客戶端

從[下載頁面](/zh_tw/download)，下載適合您的玩家的軟體包：
...
- 對於基岩專用伺服器，請使用 [McHttp for BDS](/zh_tw/minecraft/mchttp-bds)。
- 對於本地基岩世界，請使用 [McWss for Singleplayer Worlds](/zh_tw/minecraft/mcwss-singleplayer)。
- 對於 Java + Geyser/Floodgate，請使用 [GeyserVoice](/zh_tw/ecosystem/geyservoice)。

如果您要在基岩上部署，請將這兩個頁面放在附近：

- [下載頁面](/zh_tw/download) 用於原始客戶端/伺服器/外掛程式發布文件
...
- [伺服器安裝](/zh_tw/server/installation)
- [首次伺服器運行](/zh_tw/server/first-run)
- [ServerProperties.json](/zh_tw/server/server-properties)
