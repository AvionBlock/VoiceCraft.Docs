...
Bedrock Dedicated Server deployments also need the `VoiceCraft.Addon.Core.McHttp` package or a configured world archive from the [Addon Configurator](/addon-configurator). Local Bedrock worlds usually use `VoiceCraft.Addon.Core.McWss`. Java/Geyser deployments use `GeyserVoice` plus the `McTcp` transport.
