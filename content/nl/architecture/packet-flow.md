...
Op deze pagina wordt de conceptuele stroom uitgelegd in plaats van elk pakkettype op te sommen. Het is handig als een installatie gedeeltelijk werkt: de client maakt bijvoorbeeld verbinding maar er wordt geen proximity audio afgespeeld, of de add-on maakt verbinding maar de binding wordt nooit voltooid.
...
Beide vlakken moeten gezond zijn om proximity voice correct te laten werken.
...
3. De Minecraft-transportclient authenticeert met zijn geconfigureerde token.
...
6. De server verzendt de status die verbonden clients nodig hebben.
7. Clients renderen het resulterende spraakgedrag lokaal.

De volgorde kan per topologie iets verschillen. Belangrijk is dat clientlogin en Minecraft-transportlogin afzonderlijke gebeurtenissen zijn. De ene kant kan werken terwijl de andere nog stuk is.

## Typische eventcategorieën
...
5. Positie- en wereldupdates beginnen te beïnvloeden wat de client hoort.

Typische Java/Geyser-flow:

1. `VoiceCraft.Java` maakt verbinding met `McTcp`.
...
4. `VoiceCraft.Java` verzendt de bind-/updategegevens naar VoiceCraft.
...
|---------|----------------------|---------------|
| Klant kan geen verbinding maken | Spraakvlak | Verkeerde serverhost, UDP-poort gesloten, server draait niet |
| Add-on/plug-in kan geen verbinding maken | Minecraft-statusvlak | Verkeerd transporttoken, verkeerde binding, geblokkeerd TCP/HTTP/WebSocket-pad |
| Client maakt verbinding, maar hoort geen proximity audio | Entiteit/positiestatus | Bind ontbreekt, `PositioningType` komt niet overeen, geen positie-updates |
