...
Для развертывания выделенного сервера Bedrock также необходим пакет `VoiceCraft.Addon.Core.McHttp` или настроенный архив мира из [Addon Configurator](/addon-configurator). Локальные миры Bedrock обычно используют `VoiceCraft.Addon.Core.McWss`. В развертываниях Java/Geyser используется `GeyserVoice` вместе с транспортом `McTcp`.
