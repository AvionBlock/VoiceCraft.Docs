...
`GeyserVoice` подключает инфраструктуру на стороне Java к `VoiceCraft.Server` через транспорт `McTcp`.
...
- развертывание прокси Velocity
- развертывание прокси BungeeCord
- смешанная топология прокси + бэкенд-узлы
...
- сценария, в котором плагин сам загружает и запускает VoiceCraft
...
- `voice.port`
  голосовой порт среды выполнения VoiceCraft, используемый управляемым путем запуска.
- `auto-start`
...
- в режиме прокси идентификаторы миров могут получать пространство имен с учетом бэкенда.
...
5. Протестируйте процесс привязки и обновления позиций, прежде чем открывать сервер игрокам.
