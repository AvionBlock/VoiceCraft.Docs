...
- при необходимости остановить среду выполнения при отключении плагина

Это одна из наиболее важных текущих функций для пользователей Direct Paper.
...
Используйте `config.voicecraft.transport.host`, `config.voicecraft.transport.port` и `config.voicecraft.transport.login-token` для соединения VoiceCraft `McTcp`. При внешней среде выполнения они должны соответствовать конфигурации `VoiceCraft.Server`.
...
- клиент подключается, но состояние Java не влияет на звук с учетом расстояния:
  проверьте `/voice bind`, интервал обновления позиции и режим позиционирования на стороне сервера.

## Контрольный список проверки

- журналы Paper показывают, что GeyserVoice включен
