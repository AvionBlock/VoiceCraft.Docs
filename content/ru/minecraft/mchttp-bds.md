...
Используйте это руководство, если вы запускаете выделенный сервер Bedrock и хотите, чтобы серверное дополнение отправляло состояние игрока в `VoiceCraft.Server`.
...
- хорошо сочетается с пакетом дополнений Bedrock `VoiceCraft.Addon.Core.McHttp`
- не зависит от локального рабочего процесса веб-сокета `/connect`, используемого `McWss`
...
- убедитесь, что хост BDS может достичь настроенной конечной точки
...
Надстройке требуются разрешения для сценариев, связанных с сетью, поскольку она вызывает конечную точку HTTP VoiceCraft из среды выполнения BDS.
...
- поток привязки становится доступным через `voicecraft:vcbind`
- Становятся доступными пользовательский интерфейс эффектов и синхронизация состояния на основе пакетов.

На этом этапе транспорт подключен, но каждому игроку по-прежнему нужен клиент VoiceCraft и работающий поток привязки для бесконтактного звука.
...
7. Переместите игрока в игру и убедитесь, что обновления местоположения влияют на близость.
...
- клиент подключается, но нет близости:
  подтвердите поток привязки, `PositioningType` и обновления позиции игрока
