...
VoiceCraft полезен, когда вам нужен близкий голос, не зависящий от одной конкретной формы сервера Minecraft. Одну и ту же основную среду выполнения можно комбинировать с аддонми Bedrock, мостами Java/Geyser или развертываниями прокси.
...
Голосовое соединение и транспортное соединение Minecraft разделены. Если подключена только одна сторона, установка может выглядеть частично работоспособной, но proximity-поведение все равно будет неполным.
...
|---------------|---------------|-----|
| Выделенный сервер Bedrock | [McHttp for BDS](/minecraft/mchttp-bds) | BDS может вызывать стабильный HTTP endpoint |
| Локальный мир Bedrock | [McWss for Singleplayer Worlds](/minecraft/mcwss-singleplayer) | Работает через локальный поток WebSocket/командного туннеля. |
