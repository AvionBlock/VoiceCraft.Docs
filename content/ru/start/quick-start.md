...
Для первой настройки выберите одну топологию и откройте только тот транспорт, который ей нужен. Смешанные схемы можно добавить позже, когда уже работают базовая привязка и proximity audio.
...
Загрузите пакет для своих игроков с [download page](/download):
...
4. Используйте UDP endpoint VoiceCraft из `VoiceCraftConfig.Port`.
...
Именно этот шаг дает VoiceCraft внутриигровое состояние, необходимое для proximity audio: личность игрока, данные привязки, мировые идентификаторы, обновления местоположения и состояние эффекта.

Если вы выполняете развертывание на Bedrock, держите эти две страницы поблизости:

- [Download Page](/download) для файлов релизов клиента, сервера и аддона
...
- игроки слышат proximity voice, когда они находятся в пределах досягаемости

Если клиент подключается, но proximity не работает, отладьте транспорт Minecraft и процесс привязки, прежде чем изменять настройки звука.
