...
- `PushToTalkCue`:
  логический флаг для локального звукового сигнала PTT.
...
Записи сервера указывают на UDP-эндпоинт VoiceCraft из `VoiceCraftConfig.Port`. Они отличаются от эндпоинтов транспортов Minecraft `McHttp`, `McWss` или `McTcp`.
...
- держите `PositioningType` согласованным с сервером
