...
Используйте эту страницу, когда вам нужно понять, чем управляет поле и какие поля должны соответствовать клиенту, дополнению или плагину.
...
`Port` — это конечная точка, которую клиенты проигрывателя добавляют в пользовательский интерфейс клиента VoiceCraft. Это не то же самое автоматически, что и каждая конечная точка транспорта Minecraft, даже если по умолчанию повторно используется `9050`.

`PositioningType` должен соответствовать настройке клиента. В большинстве настроек BDS и GeyserVoice начните с `0 = Server`.

## McWssConfig

Используется для потоков веб-сокетов/командных туннелей Bedrock.
...
- `Hostname`:
  хост веб-сокета, например `ws://0.0.0.0:9051/`.
- `MaxClients`:
...
Используйте `McHttp`, если BDS может достичь конечной точки HTTP VoiceCraft. Если BDS и VoiceCraft работают на разных машинах, `127.0.0.1` будет указывать на неправильный хост с точки зрения BDS.
...
Изменяйте их только тогда, когда вы понимаете конвейер эффектов. В большинстве случаев перед изменением эффектов по умолчанию проверьте базовое поведение привязки и близости.
