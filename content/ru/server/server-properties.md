...
Телеметрия помогает сопровождающим понять состояние среды выполнения и распространение версий. Ее не следует использовать как замену собственному мониторингу; для production-серверов ведите локальные журналы и отслеживайте процессы отдельно.
...
Используется для сценариев Bedrock на основе WebSocket и командного туннеля.
...
Используйте `McHttp`, если BDS может достичь HTTP-эндпоинта VoiceCraft. Если BDS и VoiceCraft работают на разных машинах, `127.0.0.1` будет указывать на неправильный хост с точки зрения BDS.
...
Вы можете переопределить или расширить словарь, чтобы изменить поведение эффекта по умолчанию для новых сущностей.

Изменяйте их только тогда, когда вы понимаете конвейер эффектов. В большинстве случаев перед изменением эффектов по умолчанию проверьте базовое поведение привязки и звук с учетом расстояния.
...
- отключение основных пакетов может нарушить вход в систему, синхронизацию сущностей или доставку звука.
...
## Практические production-модели
